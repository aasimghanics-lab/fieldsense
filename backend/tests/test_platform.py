...
def payload(value=25):
    return {'id': 'test-'+str(uuid.uuid4()), 'sensor_id': 'sensor-0-0-0', 'timestamp': datetime.now(timezone.utc).isoformat(), 'value': value, 'raw': {'nested': {'probe': 'test'}}}
...
def test_health(client):
    assert client.get('/api/health').json() == {'status':'ok','postgres':'ok','postgis':'ok','mongo':'ok'}


@pytest.mark.parametrize('path', ['farms','fields','plots','crops','experiments','sensors','treatments','seasons','weather','notes'])
...
def test_pagination_filter_aggregation(client):
    ...
    assert client.get('/api/readings?start=2026-02-01T00:00:00Z&end=2026-01-01T00:00:00Z').status_code == 422
...
def test_geospatial(client):
    ...
    with Session() as db:
        indexes = db.execute(text("SELECT indexdef FROM pg_indexes WHERE tablename IN ('fields','plots','sensors')")).scalars().all()
...
def test_anomaly_model(client, headers):
    ...
    assert 0 < result['anomalies'] < result['fitted']*.1
...
def test_experiment_validation(client, headers):
    body = {'name':'New trial','description':'Testing','crop_id':'corn','season_id':'season-demo','start_date':'2026-06-01','end_date':'2026-05-01','treatments':['Control']}
    ...
    try:
        assert client.post('/api/notes', json={'experiment_id':eid,'body':'A test annotation'}, headers=headers).status_code == 201
    finally:
        with Session() as db:
            for table in ['notes','treatments','experiments']:
                db.execute(text(f'DELETE FROM {table} WHERE {"id" if table == "experiments" else "experiment_id"}=:id'), {'id':eid})
            db.commit()
