...
def main():
    ...
    with httpx.Client(base_url=base, timeout=20) as client:
        sensors = client.get('/api/sensors', params={'size': 500}).raise_for_status().json()['items']
        while True:
            batch = [{'id': str(uuid.uuid4()), 'sensor_id': s['id'], 'timestamp': datetime.now(timezone.utc).isoformat(), 'value': random.uniform(20, 35), 'raw': {'source': 'continuous simulator'}} for s in sensors if s['type_id'] == 'soil_moisture']
