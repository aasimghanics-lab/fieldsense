...
from .models import Field as ResearchField, Plot, Sensor, SensorType
...
def sensor_metadata(db, filters):
    query = select(Sensor, Plot, ResearchField, SensorType).join(Plot, Sensor.plot_id == Plot.id).join(ResearchField, Plot.field_id == ResearchField.id).join(SensorType, Sensor.type_id == SensorType.id)
    for key, column in {'farm': ResearchField.farm_id, 'field': Plot.field_id, 'plot': Plot.id, 'crop': Plot.crop_id, 'experiment': Plot.experiment_id, 'sensor': Sensor.id, 'measurement': SensorType.measurement}.items():
        if getattr(filters, key):
            query = query.where(column == getattr(filters, key))
    return {s.id: dict(sensor_id=s.id, sensor=s.name, plot=p.name, plot_id=p.id, field=f.name, field_id=f.id, farm_id=f.farm_id, crop_id=p.crop_id, experiment_id=p.experiment_id, treatment_id=p.treatment_id, measurement=t.measurement, unit=t.unit) for s, p, f, t in db.execute(query)}
...
def detect(sensor_id):
    rows = list(documents.readings.find({'sensor_id': sensor_id, 'value': {'$ne': None}}, {'value': 1}).sort('timestamp', -1).limit(10000))
    if len(rows) < 30:
    ...
    documents.readings.bulk_write([UpdateOne({'_id': r['_id']}, {'$set': {'anomaly': bool(label == -1), 'anomaly_score': float(score), 'model': 'isolation-forest-v1'}}) for r, label, score in zip(rows, labels, scores)])
    return {'fitted': len(rows), 'anomalies': int(sum(labels == -1)), 'model': 'isolation-forest-v1'}
